...
# Load combined trawl survey data, habitat variables, and species metadata
load("E:/TNC_computer/Documents/coca/Data/alltrawlnn.RData")
load('E:/TNC_computer/Documents/coca/Data/habitatvars2.RData')
species_conversions = read_excel("E:/TNC_computer/Documents/coca/BellR_species_listConversions.xlsx")
...
# Calculate patch perimeter and area correlation and select one for each species model use
# In the end, which was chosen had very little impact on model results, so you could easily
# use perimeter or area for all species, or simply remove patch dynamics altogether. 
for (Seas in Season){
...
for (spp in u_svspp[c(1:25)]){
  ...
  fishdata = alltrawl[which(alltrawl$SVSPP==spp),]# & alltrawl$SEASON==Seas),];
...
# Pull previously calculated temperature coefficents
{
predcoefs.fall = read.csv("E:/TNC_computer/Documents/coca/Data/Tempcurves_2020/coefs4.csv")
predcoefs.spring = read.csv("E:/TNC_computer/Documents/coca/Data/Tempcurves_2020/coefs2.csv")
if (Season==2){predicted.coefs = predcoefs.spring; seasname='Spring'} 
if (Season==4){predicted.coefs = predcoefs.fall; seasname = 'Fall'}
}
...
for (spp in u_svspp){ 
  ...
  habcval.df = data.frame(matrix(data=NA,nrow=5,ncol=20)); names(habcval.df) = c('SVSPP','Season','partition','modelnumber','AIC','dev.exp','r.sq','auc','maxtss_thresh','maxtss_sens','maxtss_spec','prev_thresh','prev_sens','prev_spec','mult_cor','maxtss_cor','prev_cor','mult_rmse','maxtss_rmse','prev_rmse')  
  habcval.df$partition = 1:5; habcval.df$SVSPP = spp; habcval.df$Season = Season
  ...
  for (part in 1:5){
    ...
    write.csv(habcv.part,file=paste('E:/TNC_computer/Documents/coca/Data/cval',Season,'/spp',spp,'/habCVpartHB',part,'.csv',sep=''),row.names = FALSE)
    
    for (i in 1:nrow(habcv.part)){ # Eliminate models containing insignificant variables
        if(any(is.numeric(habcv.part[i,2:6]))){habcv.part[i,'AIC'] = NA}}
...
# TSS CV
evaluation_df = data.frame(matrix(NA,25,10))
names(evaluation_df) = c('SVSPP','Season','Sensitivity','SD_Sensitivity','Specificity','SD_Specificity','TSS','SD_TSS','AUC','SD_AUC')
component_eval = data.frame(matrix(NA,25,8))
names(component_eval) = c('SVSPP','Season','TSS_overall','TSS_temp','TSS_benthic','AUC_overall','AUC_temp','AUC_benthic')


for (spp in u_svspp[c(1:25)]){ 
  print(spp)
  cv_tss = data.frame(matrix(data=NA,nrow=5,ncol=11))
  names(cv_tss) = c('part','thresh','sens','spec','tss','R2','AUC','tss_temp','tss_benth','AUC_temp','AUC_benth')
  habcval.df = read.csv(paste('E:/TNC_computer/Documents/coca/Data/cval',Season,'/spp',spp,'/habCValDFHB',spp,'.csv',sep=''),header = TRUE)
  ind.vec = which(u_svspp==spp)
  fishdata = alltrawl[which(alltrawl$SVSPP==spp & alltrawl$SEASON==Season),];
  fishdata[which(fishdata$sedclasses==128),'sedclasses'] = NA;
  if(patchcor[ind.vec,'usepatch']=='Perim'){fishdata$Patch = fishdata$PatchPerim; habvars2$Patch = habvars2$PatchPerim}
  if(patchcor[ind.vec,'usepatch']=='Area'){fishdata$Patch = fishdata$PatchArea; habvars2$Patch = habvars2$PatchArea}
  i.na = which(is.na(fishdata[,resp])|is.na(fishdata$BTEMP)|is.na(fishdata$PatchArea)|is.na(fishdata$PatchPerim)|is.na(fishdata$PatchCompact)|is.na(fishdata$bathyclasses)|is.na(fishdata$bpibroadclasses)|is.na(fishdata$sedclasses)|is.na(fishdata$HB))
  if(length(i.na)>0){
    fishdata = fishdata[-i.na,]}
  
  threshpart = data.frame(matrix(NA,5,3)); names(threshpart) = c('spp','season','thresh')
  for (part in 1:5){
    fishtrain = fishdata[which(fishdata$partition!=part),]
    fishtest = fishdata[which(fishdata$partition==part),]
  
  
    if (Season==2){jlcoefs = read.csv(paste('E:/TNC_computer/Documents/coca/Rich_GAMS/tmb/springcoefs/tmbcoefs',spp,'.csv',sep=''),header=TRUE);}
    if (Season==4){jlcoefs = read.csv(paste('E:/TNC_computer/Documents/coca/Rich_GAMS/tmb/fallcoefs/tmbcoefs',spp,'.csv',sep=''),header=TRUE);}
    Er = jlcoefs[part,'Er']; 
    Ed = jlcoefs[part,'Ed']; 
    cc = jlcoefs[part,'cc']; 
    ...
    bestmod = habcval.df[which(habcval.df$AIC==min(habcval.df$AIC)),'modelnumber']
    gf = gflist[[bestmod]]
    ...
    # Calculates thresholds for each model componenent (temperature, benthic, combined) for evaluation purposes
    PAdata = cbind(1:nrow(fishtest),fishtest$presence,fishtest$jlsqrt_habsd,fishtest$jlpred_sqrt,testpred_sd)
    thresh = optimal.thresholds(PAdata, which.model = 1)[3,2];
    thresh2 = optimal.thresholds(PAdata, which.model = 2)[3,2];
    thresh3 = optimal.thresholds(PAdata, which.model = 3)[3,2]
  
    CMX1 = cmx(PAdata, threshold = thresh, which.model = 1)
    sens = sensitivity(CMX1,st.dev=FALSE) 
    spec = specificity(CMX1,st.dev=FALSE) 
    tss = sens+spec-1 
    CMX2 = cmx(PAdata, threshold = thresh2, which.model = 2)
    sens_temp = sensitivity(CMX2,st.dev=FALSE) 
    spec_temp = specificity(CMX2,st.dev=FALSE)
    tss_temp = sens_temp+spec_temp-1
    CMX3 = cmx(PAdata, threshold = thresh3, which.model = 3)
    sens_benth = sensitivity(CMX3,st.dev=FALSE) 
    spec_benth = specificity(CMX3,st.dev=FALSE)
    tss_benth = sens_benth+spec_benth-1

    cv_tss[part,'part'] = part;
    cv_tss[part,'sens'] = sens;
    cv_tss[part,'spec'] = spec;
    cv_tss[part,'tss'] = tss;
    cv_tss[part,'thresh'] = thresh
    cv_tss[part,'R2'] = summary(gammod)$r.sq
    cv_tss[part,'AUC'] = auc(PAdata, which.model = 1)[1]
    cv_tss[part,'tss_temp'] = tss_temp; 
    cv_tss[part,'tss_benth'] = tss_benth;
    cv_tss[part,'AUC_temp'] = auc(PAdata, which.model = 2)[1]
    cv_tss[part,'AUC_benth'] = auc(PAdata, which.model = 3)[1]
    
    cmx_filepath = sprintf('E:/TNC_computer/Documents/coca/evaluation/s%d/confusion_matrices/CMX_%d_%d.csv',Season,spp,part)
    write.csv(CMX1,file=cmx_filepath,row.names = TRUE)
    threshpart[part,] = c(spp,Season,thresh)
  }
  evaluation_df[ind.vec,'SVSPP'] = spp;
  evaluation_df[ind.vec,'Season'] = Seas;
  evaluation_df[ind.vec,'Sensitivity'] = round(mean(cv_tss$sens),3)
  evaluation_df[ind.vec,'Specificity'] = round(mean(cv_tss$spec),3)
  evaluation_df[ind.vec,'TSS'] = round(mean(cv_tss$tss),5)
  evaluation_df[ind.vec,'AUC'] = round(mean(cv_tss$AUC),3)
  evaluation_df[ind.vec,'SD_Sensitivity'] = round(sd(cv_tss$sens),3)
  evaluation_df[ind.vec,'SD_Specificity'] = round(sd(cv_tss$spec),3)
  evaluation_df[ind.vec,'SD_TSS'] = round(sd(cv_tss$tss),3)
  evaluation_df[ind.vec,'SD_AUC'] = round(sd(cv_tss$AUC),3)
  
  # # Temperature and benthic specific TSS
  component_eval[ind.vec,'SVSPP'] = spp; 
  component_eval[ind.vec,'Season'] = Seas
  component_eval[ind.vec,'TSS_overall'] = round(mean(cv_tss$tss),3)
  component_eval[ind.vec,'AUC_overall'] = round(mean(cv_tss$AUC),3)
  component_eval[ind.vec,'TSS_temp'] = round(mean(cv_tss$tss_temp),3)
  component_eval[ind.vec,'AUC_temp'] = round(mean(cv_tss$AUC_temp),3)
  component_eval[ind.vec,'TSS_benthic'] = round(mean(cv_tss$tss_benth),3)
  component_eval[ind.vec,'AUC_benthic'] = round(mean(cv_tss$AUC_benth),3)
  
  thresh_filepath = sprintf('E:/TNC_computer/Documents/coca/evaluation/s%d/thresh/thresh_%d.csv',Season,spp)
  write.csv(threshpart,file=thresh_filepath,row.names = FALSE)
  
}
  
write.csv(evaluation_df,file = paste('E:/TNC_computer/Documents/coca/evaluation/cv_eval_df', Season, '.csv', sep = ''), row.names = FALSE)
write.csv(component_eval,file = paste('E:/TNC_computer/Documents/coca/evaluation/cv_compeval_df', Season, '.csv', sep = ''), row.names = FALSE)
